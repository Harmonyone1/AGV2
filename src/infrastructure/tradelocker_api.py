import datetime, json, logging, requests, jwt, pandas as pd

# pandas 3+ requires opting in to downcasting behavior
pd.set_option("future.no_silent_downcasting", True)
from functools import lru_cache
from typing import Callable, cast, Literal, Optional, DefaultDict, Any

# pull version, utils, types and exceptions from the installed tradelocker package
...
from tradelocker.types            import (
    ...
    JSONType,
    SideType,
    AccountsColumns as _AccountsColumns,
    ExecutionsColumns,
    OrdersColumns,
    PositionsColumns,
    PriceHistoryColumns,
...
# Extend column definitions with additional fields returned by the API
AccountsColumns = {**_AccountsColumns, "status": str}
...
class TLAPI:
    ...
    Provides helpers like :meth:`get_price_history`,
    :meth:`get_market_depth` and :meth:`get_volume_history` for
    ...
    @log_func
    @tl_typechecked
    def get_volume_history(self, instrument_id: int) -> pd.DataFrame:
        """Returns trading volume data for the requested instrument.
        ...
        Returns:
            pd.DataFrame: DataFrame containing instrument's volume history
        """
        route_url = f"{self.get_base_url()}/trade/volume"
        ...
        try:
            volume_details = pd.DataFrame(get_nested_key(response_json, ["d", "volumeDetails"]))
        except KeyError as err:
            if response_json["s"] == "no_data":
                self.log.warning("No data returned from the API for the given period")
                volume_details = pd.DataFrame(columns=["t", "v"])
            else:
                raise err

        self._apply_typing(volume_details, PriceHistoryColumns)
